...
void IRGenerator::HandleVarDeclArray(vector<std::shared_ptr<TopLevelDef>>& defs,
    ...
    if (v->u.varDeclStmt->u.varDecl->u.declArray->type->type ==
        ...
        ValDef def(
            RegType::kStructPtr, v->u.varDeclStmt->u.varDecl->u.declArray->len,
            *v->u.varDeclStmt->u.varDecl->u.declArray->type->u.structType);
        defs.push_back(TopLevelDef::CreateGlobalDef(
            *v->u.varDeclStmt->u.varDecl->u.declArray->id, def, vector<int>()));
    } else {
        global_vars_.emplace(
            *v->u.varDeclStmt->u.varDecl->u.declArray->id,
...
void IRGenerator::HandleVarDefScalar(vector<std::shared_ptr<TopLevelDef>>& defs,
    ...
    if (v->u.varDeclStmt->u.varDef->u.defScalar->type->type ==
        ...
        ValDef def(
            ...
            *v->u.varDeclStmt->u.varDef->u.defScalar->type->u.structType);
        defs.push_back(TopLevelDef::CreateGlobalDef(
            *v->u.varDeclStmt->u.varDef->u.defScalar->id, def, vector<int>()));
    } else {
        global_vars_.emplace(
            *v->u.varDeclStmt->u.varDef->u.defScalar->id,
            GlobalVal::CreateInt(BlockLabel::CreateFrom(
...
void IRGenerator::HandleVarDefArray(vector<std::shared_ptr<TopLevelDef>>& defs,
    ...
    if (v->u.varDeclStmt->u.varDef->u.defArray->type->type ==
        ...
        ValDef def(RegType::kStructPtr,
                   ...
                   *v->u.varDeclStmt->u.varDef->u.defArray->type->u.structType);
        defs.push_back(TopLevelDef::CreateGlobalDef(
            *v->u.varDeclStmt->u.varDef->u.defArray->id, def, vector<int>()));
    } else {
        global_vars_.emplace(
            *v->u.varDeclStmt->u.varDef->u.defArray->id,
            GlobalVal::CreateIntPtr(
...
void IRGenerator::HandleFnDecl(vector<std::shared_ptr<TopLevelDef>>& defs,
    ...
    defs.push_back(
        TopLevelDef::CreateFuncDecl(*v->u.fnDeclStmt->fnDecl->id, args, type));
}
